...
datapath=$1 #path to folder of reads i.e. SoyNAM
test_path=$2
ref_option=$3
refpath=$4
...
common_PBS="-q super_mem -l nodes=1:ppn=60,walltime=48:00:00 -A aaa -M junyuli2@illinois.edu -m ae"

cortex_path="/projects/bioinformatics/builds/CORTEX_release_v1.0.5.21/"
common_cortex="--kmer_size 63 --mem_height 29 --mem_width 75"
...
`${pbs_generator_path}/CortexWorkflow_Step1_CreateUncleanedGraphPerSample.sh ${datapath} ${test_path} ${cortex_path} "${common_cortex}" ${memprof}` #create step1 qsubs

`truncate -s 0 ${outpath}/stage1_list`

for file in ${test_path}/PBS/qsubs/*WorkflowStage1*.qsub #qsubs created in CortexWorkflow_Step1_CreateUncleanedGraphPerSample.sh
do
	step1=$(qsub ${common_PBS} ${file})    #execute qsub, step1 is assigned the jobID, common PBS can be edited depending on the job, other options already in PBS file
	echo $step1
	printf :$step1 >> ${outpath}/stage1_list #stage1 list contains all of the qsub IDs of this Cortex run
done
...
`${pbs_generator_path}/CortexWorkflow_Step2_PoolUncleanedGraphs.sh ${test_path} ${cortex_path} "${common_cortex}" ${memprof}` #create step2 qsub

all_step1=`cat ${outpath}/stage1_list`
step2=$(qsub ${common_PBS} -W depend=afterok$all_step1 ${test_path}/PBS/qsubs/CortexVar_WorkflowStage2_Pool_Wash.qsub) #afterok ensures step2 wont execute until after step1
echo $step2 
...
# echo ${outpath}/step5_binarylist.REF > ${outpath}/step5_colorlist_cleaned # moved to step5 PBS generator 
...
`${pbs_generator_path}/CortexWorkflow_Step6_PathDivergence.sh ${test_path} ${cortex_path} "${common_cortex}" ${memprof}`
