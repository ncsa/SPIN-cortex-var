...
do
       qsubfile=CortexVar_WorkflowStage6_0-i_${sample_id}.qsub
       qsubname=CV_Step6_0-i_${sample_id}
...
#========================================================             
#CORTEX:     
       echo "${memprofpath}/memprof.sh ${cortex_path} --kmer_size ${kmer_size} --mem_height ${mem_height} --mem_width ${mem_width} --multicolour_bin ${merged_clean_pool_path} --detect_bubbles1 0/${color_id} --output_bubbles1 ${workpath}/outputs/step6_variants/${sample_id}_0-i_BubbleCaller --print_colour_coverages" >> ${qsubpath}/${qsubfile}
